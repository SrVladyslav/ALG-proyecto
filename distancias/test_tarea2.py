...
def dp_levenshtein_backwards(x, y, threshold):
...
def dp_restricted_damerau_backwards(x, y, threshold):
    M = np.ones((len(x) + 1, len(y) + 1))*np.inf
    for i in range(0, len(x) + 1):
        M[i, 0] = i
    for j in range(0, len(y) + 1):
...
def dp_intermediate_damerau_backwards(x, y, threshold):
    M = np.ones((len(x) + 1, len(y) + 1))*np.inf
    for i in range(0, len(x) + 1):
        M[i, 0] = i
    for j in range(0, len(y) + 1):
